...
try
    while calciumToSpikeParams.isAnalysisDone == 0;
        ...
        w = multiWaitbar('Overall Progress','Value',0);
        
        %This part calculates the dF/f
        [deltaff,PSNR,w] = DffCalculationAfterPSNRfiltering(calciumToSpikeParams,w);
        w = multiWaitbar('Overall Progress','Value',0.33);
        ...
        %This part creates the binary spike matrix using OASIS
        [populationSpikeMatrix, populationSpikeProbability,w] = GenerateSpikeProbabilityAndBinaryMatrixByOASIS(deltaff,w);
        w = multiWaitbar('Overall Progress','Value',0.67);
        ...
        w = multiWaitbar('Overall Progress','Value',1);
        w = multiWaitbar('Overall Progress','Reset','Close');
        calciumToSpikeParams.isAnalysisDone = 1;
     end
...
try
    load('calciumToSpikeParams.mat')
    set(handles.dataPoolButton,'ForegroundColor','red','String','Data pooling running');
    pause(0.5)
    CompileSpikeData(calciumToSpikeParams)
    CompileROIData(calciumToSpikeParams)
    set(handles.dataPoolButton,'String','Data pooling complete','BackgroundColor','green','ForegroundColor','black');
    set(handles.GUIstatusBox,'String','No issues','ForegroundColor','black')
catch
...
while calciumToSpikeParams.isFallSelected == 0
    ...
    if ischar(filePath) == 1
        ...
        calciumToSpikeParams.isFallSelected = 1;
        save('calciumToSpikeParams.mat','calciumToSpikeParams')
        set(handles.FallFileSelectionButton,'Enable','on','String','Select Fall.mat file','ForegroundColor','black','BackgroundColor',[0.94 0.94 0.94])
        set(handles.isCellStatusBox,'String','isCell status:','ForegroundColor',[0.4 0.4 0.4])
...
set(handles.analyzeDataButton,'Enable','off','String','Analyze data','ForegroundColor','black','BackgroundColor',[0.94 0.94 0.94])
set(handles.FallFileSelectionButton,'Enable','off','String','Select Fall.mat file','ForegroundColor','black','BackgroundColor',[0.94 0.94 0.94])
